...
export const createStackTool = createTool({
  ...
  inputSchema: z.object({
    ...
    authtoken: z.string().default(process.env.CONTENTSTACK_AUTH_TOKEN || '').describe('Contentstack auth token (from CONTENTSTACK_AUTH_TOKEN env var)'),
    organization_uid: z.string().default(process.env.CONTENTSTACK_ORG_ID || '').describe('Organization UID (from CONTENTSTACK_ORG_ID env var)'),
...
export const createGlobalFieldTool = createTool({
  ...
  inputSchema: z.object({
    api_key: z.string().default(process.env.CONTENTSTACK_API_KEY || '').describe('Stack API key (from CONTENTSTACK_API_KEY env var)'),
    authtoken: z.string().default(process.env.CONTENTSTACK_AUTH_TOKEN || '').describe('Contentstack auth token (from CONTENTSTACK_AUTH_TOKEN env var)'),
